...
public class RNFStress extends ConfigBase {
    ...
    public void registerAll(ModConfigSpec.Builder builder) {
        builder.comment(".", RNFStress.Comments.su, RNFStress.Comments.impact)
                .push("impact");
        DEFAULT_IMPACTS.forEach((id, value) -> this.impacts.put(id, builder.define(id.getPath(), value)));
        builder.pop();

        builder.comment(".", RNFStress.Comments.su, RNFStress.Comments.capacity)
                .push("capacity");
    ...
    public static <B extends Block, P> NonNullUnaryOperator<BlockBuilder<B, P>> setImpact(double value) {
        return builder -> {
            assertFromRnF(builder);
            ResourceLocation id = RadiationFission.asResource(builder.getName());
    ...
    private static void assertFromRnF(BlockBuilder<?, ?> builder) {
        if (!builder.getOwner().getModid().equals(RadiationFission.MOD_ID)) {
            throw new IllegalStateException("Non-Radiation&Fission blocks cannot be added to RnF's config.");
        }
    }
